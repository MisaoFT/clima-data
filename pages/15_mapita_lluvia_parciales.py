...
import pandas as pd
import numpy as np
# --- TRUCO PARA EVITAR EL BLOQUEO DE DIRECTIVAS DE WINDOWS ---
import matplotlib
matplotlib.use('Agg') 
# -------------------------------------------------------------
...
st.set_page_config(layout="wide") # Modo ancho para despliegue cómodo
...
if archivo_subido is not None:
    try:
        ...
        # --- LÍMITES GEOGRÁFICOS FIJOS (Cubre todo el país incluyendo Petén) ---
        ...
        niveles = np.linspace(0, max(z.max(), 10), 10)
        contour = ax.contourf(xi, yi, zi, levels=niveles, cmap='YlGnBu', alpha=0.65)
        lineas = ax.contour(xi, yi, zi, levels=niveles, colors='white', linewidths=0.5, alpha=0.5)
        ax.clabel(lineas, inline=True, fmt='%.0f mm', fontsize=6, colors='black')
        
        gdf.plot(ax=ax, color='black', markersize=10, marker='o', zorder=5)
        
        for idx, row in gdf.iterrows():
            if row['Lluvia_Acumulada_mm'] > 20.0:
                ax.text(row.geometry.x, row.geometry.y + 1200, f"{row['Estación']}\n({row['Lluvia_Acumulada_mm']:.1f} mm)", 
                        fontsize=5.5, weight='bold', ha='center', zorder=6)
        ...
        ax.text(
            0.02, -0.02, 
            "Mapa preliminar generado automaticamente por la Sección de Climatología - INSIVUMEH-", 
        ...
        with col_lista:
            st.markdown("### 🏆 Ranking Estaciones")
        ...
        st.download_button("💾 Descargar Mapa (PNG)", data=buf.getvalue(), file_name="mapa_isoyetas.png", mime="image/png")
