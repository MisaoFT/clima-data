...
from datetime import datetime  # <-- Importación necesaria para el nombre dinámico
...
if subido_icc is not None:
    ...
    try:
        ...
        # Ordenamos las llaves del diccionario de la más larga a la más corta para evitar falsos positivos de sub-cadenas
        nombres_estandar_ordenados = sorted(list(mapeo_columnas.keys()), key=len, reverse=True)
        ...
        for fecha_v in fechas_unicas:
            ...
            for _, r_dato in datos_dia.iterrows():
                ...
                # Evaluación Paso A: ¿Es una coincidencia exacta de nombre?
                if estacion_raw in mapeo_columnas:
                    estacion_match = estacion_raw
                else:
                    # Evaluación Paso B: Si no es exacta, busca por sub-cadenas de mayor a menor longitud
                    for nombre_estandar in nombres_estandar_ordenados:
        ...
        # GENERACIÓN DINÁMICA DEL NOMBRE CON FECHA Y HORA DEL SISTEMA
        ahora_descarga = datetime.now()
        nombre_dinamico_excel = f"datos_lluvias_parciales_ICC_{ahora_descarga.strftime('%Y%m%d_%H%M')}.xlsx"
        ...
        st.download_button(
            ...
            data=buffer_excel,
            file_name=nombre_dinamico_excel,
            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
        )
        # --- LIMPIEZA DE MEMORIA POST-PROCESAMIENTO (ICC) ---
        del df_raw
        del df_resumen
        del datos_dia
